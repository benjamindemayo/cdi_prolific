...
maternal_ed_plot <- 
  ...
  theme(
    legend.title = element_blank(),
    axis.text.x = element_text(angle = 40, vjust = 0.9, hjust = 1, size = 13.5),
    axis.title.x = element_blank(),
    legend.text = element_text(size = 13),
    plot.caption = element_text(hjust = 0)
  ) +
...
#Words and sentences exclusions
...
ws_exclusions <- 
  ...
  getCompletionInterval() %>% 
  mutate(
    exclusion_not_first_admin = as.numeric(repeat_num != 1),
    exclusion_birthweight = as.numeric(premature),
    exclusion_multilingual = 
      as.numeric(!(is.na(language_hours_per_week)) & 
                   language_hours_per_week >= 16),
  ...
  mutate(
    quantity_exclusions = select(
      ., 
      exclusion_not_first_admin:exclusion_age_range
    ) %>% 
      rowSums()
  ) %>% 
  getEthnicities() %>% 
...
#total amount of kids excluded from WS analyses
total_n_ws_exclusions <- 
  ws_exclusions %>% 
  filter(quantity_exclusions > 0) %>% 
  nrow()

counting_ws_exclusions_ <- 
  ws_exclusions
...
#total amount of kids excluded from WG analyses
total_n_wg_exclusions <- 
  wg_exclusions %>% 
  filter(quantity_exclusions > 0) %>% 
  nrow()

nrow(wg_exclusions %>% filter(quantity_exclusions == 0))
