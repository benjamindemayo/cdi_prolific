...
facebook_wg_cutoffs %>%
  ...
  rename(
    `Age in months` = age,
    `Minimum time to completion (minutes)` = minimum_time
  ) %>% 
  kable(align = "l", caption = "Minimum times to completion, WG measure")
```
...
prolific_ws_cutoffs %>%
  ...
  kable(align = "l", caption = "Minimum times to completion, WS measure")
```
...
Participants’ educational attainment level, as measured by the primary caregiver's highest educational level reached^[Maternal education level is a common measure of family socioeconomic status, but we probe *primary caregiver* education level here to accommodate family structures in which child-rearing may not primarily be the responsibility of the child's mother.], was similarly skewed. Over 80% of children in Dataset 1 came from families with college-educated primary caregivers compared to 43% from the same group in the 2007 norms (Figure \ref{fig:demobarfig}). Furthermore, less than 1% of participants report a primary caregiver education level less than a high school degree, compared to 7% from the same group in the 2007 norms. The overrepresentation of white, non-Hispanic Americans and those with high levels of education attainment points to a general challenge encountered in vocabulary development research, which we return to when we detail our efforts to recruit more diverse participants.
...
wgglm <- 
  ggplot(
  wg_momed, 
  aes(age, words, color = maternal_ed)
) +
  facet_grid(~measure) +
...
```{r}
#Models to compare wordbank to Web-CDI for WG
...
wg_wordbank_models <- 
  wg_wordbank %>%
  mutate(words_false = wg_total_words - produced) %>%
  group_by(sample) %>%
  nest() %>%
  mutate(model = map(data, function(sample_data) {
    glm(cbind(produced, words_false) ~ age * maternal_ed - maternal_ed,
        family = "binomial", data = sample_data)
  }))
  
data_cells_wg <- wg_wordbank %>% distinct(maternal_ed, age)
...
wg_wordbank_glm_plot <- 
  ggplot(wg_wordbank, aes(age, produced, color = maternal_ed)) +
  facet_grid(~sample) +
  geom_jitter(alpha = 0.2, width = 0.225) +
  geom_line(aes(y = fitted), data = wg_wordbank_fits, size = 1.2) +
  coord_cartesian(ylim = c(0, 396)) +
  ggthemes::theme_few() +
  labs(
    x = "Age in months", 
    ...
    color = "Primary caregiver education",
    linetype = "Primary caregiver education",
    title = "Words Produced, Words and Gestures (WG)"
  ) +
  scale_x_continuous(breaks = seq(from = 8, to = 18, by = 2))

ggsave(
  "wg_wordbank_glm_plot.png",
  plot = wg_wordbank_glm_plot,
  path = fig_directory
)
...
```{r}
#Models and plot to compare Web-CDI to Wordbank for WS
...
data_cells_ws <- ws_wordbank %>% distinct(maternal_ed, age)
...
ws_wordbank_glm_plot <- 
  ggplot(ws_wordbank, aes(age, produced, color = maternal_ed)) +
  facet_grid(~sample) +
  geom_jitter(alpha = 0.2, width = 0.225) +
  geom_line(aes(y = fitted), data = ws_wordbank_fits, size = 1.2) +
  ...
  labs(
    ...
    color = "Primary caregiver education",
    linetype = "Primary caregiver education",
    title = "Words Produced, Words and Sentences (WS)"
  ) +
  scale_x_continuous(breaks = seq(from = 16, to = 30, by = 2))


ggsave(
  "ws_wordbank_glm_plot.png",
  plot = ws_wordbank_glm_plot,
  path = fig_directory
)
```
